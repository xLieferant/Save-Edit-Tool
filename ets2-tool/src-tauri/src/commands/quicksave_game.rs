...
pub async fn quicksave_game_info() -> Result<GameDataQuicksave, String> {
    ...
    let truck_id = match &player_my_truck {
        Some(t) => t.clone(),
        None => "".to_string(),
    };
    ...
    // Falls Truck gefunden, Vehicle-Daten extrahieren
    let (license_plate, odometer, trip_fuel_l) = if let Some(truck) = player_truck_info {
        // Regex für Vehicle-Block
        ...
        (license_plate_val, odometer_val, trip_fuel_val)
    } else {
        (None, None, None)
    };
    ...
    bank_id,
    player_xp,
    player_my_truck: player_my_truck.clone(), // <--- clone hier
    player_my_trailer,
    adr,
    long_dist,
    heavy,
    ...
    urgent,
    mechanical,
    vehicle_id: player_my_truck.clone(),      // <--- clone hier
