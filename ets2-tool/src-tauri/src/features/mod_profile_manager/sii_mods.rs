use super::models::{
    ActiveModBlockSnapshot, ReplaceActivePresetModsResult, ValidateActivePresetModsResult,
};
use chrono::Local;
...
pub fn replace_active_preset_mods_in_game_sii(
    ...
    new_block_lines.extend(
        expected_mod_refs
            .iter()
            .enumerate()
            .map(|(index, mod_ref)| format!("{indent}{preferred_field_name}[{index}]: \"{mod_ref}\"")),
    );
...
pub fn validate_active_preset_mods_in_game_sii(
    ...
    let expected_mod_refs = build_expected_mod_refs(expected_workshop_mod_ids, workshop_ref_prefix)?;
    let order_matches = parsed.actual_indices == (0..parsed.actual_mod_refs.len()).collect::<Vec<_>>()
...
fn build_expected_mod_refs(workshop_mod_ids: &[String], prefix: &str) -> Result<Vec<String>, String> {
    workshop_mod_ids
        .iter()
...
fn parse_active_mod_sections(body: &str) -> Result<ParsedActiveModSections, String> {
    ...
    for line in body.lines() {
        ...
        if let Some(captures) = entry_regex.captures(line) {
            ...
            let mod_ref = captures
                ...
                .map(|match_| match_.as_str().to_string())
                .unwrap_or_default();
            entries.entry(field_name).or_default().push((index, mod_ref));
        }
    }
    ...
    let actual_indices = preferred_entries.iter().map(|(index, _)| *index).collect::<Vec<_>>();
    ...
    let other_mod_refs = ACTIVE_MOD_FIELDS
        ...
        .flat_map(|field_name| {
            let mut field_entries = entries
                .remove(*field_name)
                .unwrap_or_default();
            field_entries.sort_by_key(|(index, _)| *index);
            field_entries.into_iter().map(|(_, mod_ref)| mod_ref).collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();
...
fn active_mod_entry_regex() -> Result<Regex, String> {
    Regex::new(
        r#"^([ \t]*)(active_mods|actived_mods|mod_activated)\[(\d+)\]:\s*"([^"]*)"\s*$"#,
    )
    .map_err(|error| error.to_string())
}
...
fn write_text_atomic(path: &Path, content: &str) -> Result<(), String> {
    ...
    fs::write(&tmp_path, content)
        .map_err(|error| format!("Failed to write temporary file {}: {}", tmp_path.display(), error))?;
...
fn replace_file(tmp_path: &Path, target_path: &Path) -> Result<(), String> {
    ...
    if moved == 0 {
        return Err(format!("Atomic replace failed for {}", target_path.display()));
    }
